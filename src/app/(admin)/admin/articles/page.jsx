...
function ImageModal({ item, onClose, onUpdate }) {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const result = await updateArticle(item._id || item.id, fd);
      sonnerToast.success("Article image updated");
      onUpdate(result.article || result);
      onClose();
    } catch (error) {
  ...
  return (
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      ...
      >
        ...
        <h3 className="text-xl font-bold text-red-500 mb-2">Delete Article</h3>
...
const articleSchema = z.object({
  ...
  summary: bilingualString("Summary"),
  content: bilingualString("Content"),
...
function AddModal({ onSave, onClose }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      articleSchema.parse({ ...form, image: 'present' });
      setIsSubmitting(true);

      const fd = new FormData();
      fd.append('title', JSON.stringify(form.title));
      fd.append('date', form.date);
      fd.append('description', JSON.stringify(form.summary));
      fd.append('fullDescription', JSON.stringify(form.content));
      fd.append('author', form.author);
      fd.append('image', imageFile);

      const result = await createArticle(fd);
      sonnerToast.success('Article added successfully!');
      onSave(result.article || result);
    } catch (error) {
      if (error instanceof z.ZodError) {
        const msg = error.issues?.map(e => e.message).join(', ') || 'Validation error';
        setToast({ type: 'error', message: msg });
      } else {
        setToast({ type: 'error', message: error.message || 'Failed to create article' });
      }
    } finally {
    ...
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      ...
      >
        <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-border">
          <h3 className="text-lg font-bold text-primary">Add New Article</h3>
        ...
        <div className="p-6 space-y-4">
          {[
            { label: 'Title', key: 'title', placeholder: 'Article title...', isBilingual: true },
            { label: 'Date', key: 'date', placeholder: 'e.g. 2026-05-06', type: 'date' },
            { label: 'Author', key: 'author', placeholder: 'e.g. John Doe' },
          ...
          {[
            { label: 'Summary', key: 'summary', rows: 2, placeholder: 'Brief summary...', isBilingual: true },
            { label: 'Content', key: 'content', rows: 5, placeholder: 'Full article content...', isBilingual: true },
...
function DetailView({ item, onBack, onDelete, onSave }) {
  ...
  const initData = { ...item };
  ['title', 'summary', 'content'].forEach(k => {
    let targetKey = k === 'summary' ? 'desc' : k === 'content' ? 'fullContent' : k;
  ...
  const handleSave = async () => {
    try {
      ...
      const result = await updateArticle(item._id || item.id, fd);
      sonnerToast.success('Article updated successfully!');
      onSave(result.article || result);
      setEditMode(false);
    } catch (error) {
      sonnerToast.error(error.message || 'Failed to update article');
    } finally {
      setIsSaving(false);
    }
  };
    ...
    >
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        {!editMode ? (
          <button onClick={() => setEditMode(true)} className="flex items-center gap-2 px-4 py-2 border border-accent text-accent rounded-lg text-sm font-semibold hover:bg-accent/5 transition-colors">
            <TbPencil size={15} /> EDIT ARTICLES
          </button>
        ) : (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-white rounded-2xl shadow-sm p-5 space-y-3">
            ...
            <div>
              <button onClick={onDelete} className="w-full hover:py-2 hover:px-4 border-none rounded flex items-center justify-between text-[#ff502e] hover:bg-red-50/50 transition-colors bg-white cursor-pointer group">
                <div className="flex items-center gap-3 text-sm font-medium"><TbTrash className="text-[1.2rem]" /><span>Delete Article</span></div>
        ...
        <div className="lg:col-span-2 space-y-4">
          <h3 className="text-lg font-bold text-primary">Article Details</h3>
          {/* Author */}
          <div className="bg-white rounded-2xl shadow-sm p-6">
            <h4 className="font-bold text-primary text-sm uppercase tracking-wider mb-4">Author {editMode && <span className="text-accent ml-1">({lang.toUpperCase()})</span>}</h4>
            {editMode ? (
              <input type="text" value={data.author} onChange={set('author')} className={inputCls} placeholder="Author..." />
            ) : (
              <p className="text-sm text-muted-foreground leading-relaxed">{data.author || '—'}</p>
            )}
          </div>

          {/* Summary */}
          <div className="bg-white rounded-2xl shadow-sm p-6">
            <h4 className="font-bold text-primary text-sm uppercase tracking-wider mb-4">Summary {editMode && <span className="text-accent ml-1">({lang.toUpperCase()})</span>}</h4>
            {editMode ? (
              <textarea rows={3} value={data.desc[lang]} onChange={setBilingual('desc')} className={inputCls} placeholder="Summary..." />
            ) : (
          ...
          {/* Content */}
          <div className="bg-white rounded-2xl shadow-sm p-6">
            <h4 className="font-bold text-primary text-sm uppercase tracking-wider mb-4">Content {editMode && <span className="text-accent ml-1">({lang.toUpperCase()})</span>}</h4>
            {editMode ? (
              <textarea rows={6} value={data.fullContent[lang]} onChange={setBilingual('fullContent')} className={inputCls} placeholder="Content..." />
            ) : (
...
export default function ManageArticlesPage() {
  ...
  const fetchArticles = async () => {
    try {
      ...
      sonnerToast.error('Failed to fetch articles');
    } finally {
      setLoading(false);
    }
  };
  ...
  const filtered = articles.filter(
    (a) =>
      ...
      getText(a.summary).toLowerCase().includes(search.toLowerCase()) ||
  ...
  const handleConfirmDelete = async () => {
    try {
      setIsDeleting(true);
      await deleteArticle(deleteItem._id || deleteItem.id);
        ...
        {selected ? (
          ...
          <motion.div key="table" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-6">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              ...
              >
                <TbPlus size={20} /> Add New Article
              </button>
            </div>
                  ...
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="bg-muted/50 border-b border-border">
                        {['Date', 'Title', 'Image', 'Summary', 'Actions'].map((h, i) => (
                    ...
                    <tbody className="divide-y divide-border bg-white">
                      ...
                      {loading ? (
                        <tr><td colSpan={5} className="py-10 text-center text-muted-foreground italic">Loading articles...</td></tr>
                      ) :
                        filtered.length === 0 ? (
                          <tr><td colSpan={5} className="py-10 text-center text-muted-foreground italic">No articles found.</td></tr>
                        ) :
                          paginated.map((article) => (
                            <tr key={article._id || article.id} className="hover:bg-muted/20 transition-colors">
                              ...
                              <td className="px-6 py-4 font-semibold text-primary text-sm max-w-[200px] truncate">{getText(article.title)}</td>

                              <td className="px-6 py-4">
                                <button
                                  onClick={() => setImageItem(article)}
                                  className="flex items-center gap-2 text-accent hover:text-accent/70 transition-colors group"
                                >
                                  <span className="text-sm font-medium">View Image</span>
                                </button>
                              </td>
                              <td className="px-6 py-4 text-sm text-muted-foreground max-w-xs truncate">{getText(article.summary)}</td>
